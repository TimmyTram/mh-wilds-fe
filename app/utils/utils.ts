...
export const sortCharmDataByName = (data: CharmSet[], language: string) => {
    const locale = ISO639_1[language] || 'en'; // Default to 'en' if the language is not found

    // sorted the data based on the name of the first charm in the ranks array
    return data.sort((a, b) => {
        const nameA = a.ranks[0]?.name || "";  // Default to empty string if name is missing
        const nameB = b.ranks[0]?.name || "";  // Default to empty string if name is missing
